...
def simulate_construction_status(properties: pd.DataFrame) -> pd.DataFrame:
    """Every non-plot property gets a simulated build status. Older properties
    are almost certainly ready-to-move; newer ones may still be under
    construction with a promised-vs-actual stage gap."""
    rows = []
    for _, p in properties.iterrows():
        if p["property_type"] == "Plot/Land":
            rows.append({"property_id": p["property_id"], "under_construction": False,
                          "actual_stage": 5, "promised_stage": 5, "schedule_delta": 0})
            continue
        age = p["age_years"]
        if age >= 3:
            ...
            lag_roll = RNG.random()
            if lag_roll < 0.60:
                actual_stage = promised_stage
            elif lag_roll < 0.85:
                actual_stage = max(1, promised_stage - 1)
            else:
                actual_stage = max(1, promised_stage - RNG.integers(2, 4))
        rows.append({
...
def compute_unified_score(graph: pd.DataFrame, weights=None) -> pd.DataFrame:
    ...
    raw = (
        ...
        + rt * weights["trust"]
        + g.loc[has_rent, "delivery_score"] * weights["delivery"]
