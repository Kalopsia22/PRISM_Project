...
if __name__ == "__main__":
    ...
    # persist held-out test metrics so the Streamlit page can show honest,
    # non-hardcoded performance numbers without retraining in-app
    ...
    metrics = pd.DataFrame([{
        "accuracy": accuracy, "precision": precision, "recall": recall, "f1": f1, "auc": results["auc"],
        "true_negative": int(tn), "false_positive": int(fp),
        "false_negative": int(fn), "true_positive": int(tp),
    }])
    metrics.to_csv("/home/claude/prism/data/fraud_model_metrics.csv", index=False)
