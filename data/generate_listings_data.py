...
SCAM_RING_NUMBERS = [f"+91-9{RNG.integers(100000000, 999999999)}" for _ in range(6)]
...
def build_listings(df_properties: pd.DataFrame, fraud_rate=0.12) -> pd.DataFrame:
    ...
    # Organic template reuse: in real listings, units from the same builder
    # project in the same locality/BHK often get marketed with near-identical
    # copy — one agent's template gets reused by a few colleagues for sibling
    # units. This is legitimate, not fraud. Without it, "any description
    # reuse at all" becomes a perfect fraud tell, which is unrealistically
    # clean — only *excessive* reuse (the injected pattern below) should read
    # as suspicious.
    df["_group_key"] = df["city"] + "|" + df["locality"] + "|" + df["bhk"].astype(str) + "|" + df["listing_type"]
    group_sizes = df.groupby("_group_key").size()
    eligible_groups = group_sizes[group_sizes >= 4].index.tolist()
    n_template_groups = max(1, int(len(eligible_groups) * 0.05))
    template_groups = RNG.choice(eligible_groups, size=min(n_template_groups, len(eligible_groups)), replace=False)
    for g in template_groups:
        idx = df.index[df["_group_key"] == g].tolist()
        n_share = min(len(idx), int(RNG.integers(2, 4)))
        share_idx = RNG.choice(idx, size=n_share, replace=False)
        shared_desc = df.loc[share_idx[0], "description"]
        df.loc[share_idx, "description"] = shared_desc
    ...
    # 3. Fake/ghost listing (impossible carpet-area-to-BHK ratio). Photo count
    # skews low (scammers often reuse a couple of stolen/stock photos rather
    # than zero) but overlaps the normal range — a believable fraud detector
    # can't rely on "no photos" alone, since some are lazy rather than fake.
    for idx in chunks[2]:
        bhk = max(1, df.loc[idx, "bhk"])
        fake_area = bhk * RNG.uniform(120, 180)
        df.loc[idx, "carpet_area_sqft"] = round(fake_area, 1)
        df.loc[idx, "num_images"] = int(RNG.choice(
            range(8), p=[0.20, 0.18, 0.15, 0.13, 0.12, 0.10, 0.07, 0.05]
        ))
        df.loc[idx, "is_fraud"] = 1
        df.loc[idx, "fraud_type"] = "fake_listing"

    # 4. Broker scam-ring reuse (a minority keep a regular number — real scam
    # operations aren't perfectly disciplined either, so raw reuse count
    # alone shouldn't be a 100% tell)
    for idx in chunks[3]:
        if RNG.random() < 0.8:
    ...
    # 5. Recycled/templated description — several distinct scam operations,
    # not one single mega-bot, so it reads as multiple moderate clusters
    # rather than one implausibly huge one
    recycled_templates = [
        DESC_TEMPLATES_SALE[0].format(bhk=2, locality="Prime Location", builder="Reputed Builder", floor=5, facing="North", age=3),
        "Best deal guaranteed! Contact now for site visit. Genuine buyers only, no brokers.",
        "Investment opportunity — high ROI property. Limited units left. Call today.",
        DESC_TEMPLATES_RENT[0].format(bhk=2, locality="Prime Location", floor=3, facing="South"),
    ]
    recycled_chunk_groups = np.array_split(chunks[4], len(recycled_templates))
    for template, idx_group in zip(recycled_templates, recycled_chunk_groups):
        df.loc[idx_group, "description"] = template
    for idx in chunks[4]:
        df.loc[idx, "is_fraud"] = 1
