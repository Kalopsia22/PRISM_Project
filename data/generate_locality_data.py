...
MICRO_MARKETS = [
    ...
    ("Kolkata", "Rajarhat", "700135", 5000, 7500, 2.5, "affordable", "Tier1"),
...
def apply_real_calibration(markets: list, calibration_path: str = "/home/claude/prism/data/real_locality_calibration.csv") -> list:
    """Replace a locality's illustrative (rr_min, rr_max) circle-rate band
    with one grounded in real observed prices wherever a real, uploaded
    scraped-listings dataset has enough matching rows (n >= 10) for that
    (city, locality). Localities with no real match keep their original
    illustrative band unchanged.

    Real listing prices are FINAL asking prices, already including whatever
    amenity/builder/age/metro premiums exist in that market — but rr_min/
    rr_max here represent the generator's BASE circle rate, which then gets
    multiplied by those same premium factors during generation (see
    generate_properties: price_per_sqft = rr_rate * type_mult *
    (1+amenity_premium) * (1+metro_premium*0.15) * builder_premium *
    age_discount). Using real prices as the base rate directly would double
    -count that markup — verified empirically: the generator's own average
    combined premium multiplier for Apartments is ~1.337x (computed both
    from the feature formula and from the actual price/circle_rate ratio in
    generated data, which agree to 3 decimal places). Real percentiles are
    deflated by that factor before being used as rr_min/rr_max, so the
    generator's own markup reconstructs the real observed price level
    ...
    AVG_PREMIUM_MULTIPLIER = 1.337  # empirically derived, see docstring

    calib = pd.read_csv(calibration_path)
    calib_lookup = {(row["city"], row["locality_lower"]): (row["real_p25"], row["real_p75"])
                     for _, row in calib.iterrows()}

    updated = []
    n_calibrated = 0
    for city, locality, pincode, rr_min, rr_max, metro_km, tier, city_tier in markets:
        key = (city, locality.lower())
        if key in calib_lookup:
            real_p25, real_p75 = calib_lookup[key]
            rr_min = round(real_p25 / AVG_PREMIUM_MULTIPLIER)
            rr_max = round(real_p75 / AVG_PREMIUM_MULTIPLIER)
            n_calibrated += 1
        updated.append((city, locality, pincode, rr_min, rr_max, metro_km, tier, city_tier))

    if n_calibrated:
        print(f"Real-data calibration: {n_calibrated}/{len(markets)} localities recalibrated from real listings")
    return updated


MICRO_MARKETS = apply_real_calibration(MICRO_MARKETS)
