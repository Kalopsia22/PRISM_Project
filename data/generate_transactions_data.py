...
def build_transactions(properties: pd.DataFrame, entities: pd.DataFrame, flag_rate=0.13) -> pd.DataFrame:
    ...
    # 1. Undervaluation (widened to overlap the clean baseline's upper tail —
    # a hard threshold with no overlap would make this trivially separable)
    for idx in chunks[0]:
        df.loc[idx, "declared_value"] = round(df.loc[idx, "fair_value"] * RNG.uniform(0.68, 0.88), 0)
    ...
    # 3. High cash component (widened low end to overlap the clean baseline's
    # tail — some legitimate deals do have elevated-but-not-crazy cash use)
    for idx in chunks[2]:
        df.loc[idx, "cash_component_pct"] = round(RNG.uniform(11, 60), 1)
        df.loc[idx, "aml_flag"] = 1
    ...
    # Label noise: real AML ground truth is never perfectly clean — STR
    # determinations involve contested analyst judgment, some genuinely
    # suspicious deals never get flagged (missed review capacity), and some
    # flagged deals turn out to have an innocent explanation on follow-up.
    # Without this, a classifier trained on purely rule-generated labels can
    # reconstruct the injection rule almost exactly, which reads as
    # suspiciously perfect. ~3% of labels are flipped in each direction.
    noise_idx = RNG.choice(df.index, size=int(len(df) * 0.03), replace=False)
    df.loc[noise_idx, "aml_flag"] = 1 - df.loc[noise_idx, "aml_flag"]
...
if __name__ == "__main__":
    ...
    entities = build_entity_pool(n_individuals=len(props) * 4, n_shell_ring=15, n_structuring_pool=25)
