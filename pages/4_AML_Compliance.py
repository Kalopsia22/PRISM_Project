...
def load_data():
    ...
    metrics = pd.read_csv(os.path.join(BASE, "data", "aml_model_metrics.csv")).iloc[0]
    return txns, metrics
...
section_title("🌡️", "Risk heatmap: city × buyer entity type")
props_lookup = pd.read_csv(os.path.join(BASE, "data", "properties.csv"))[["property_id", "city"]]
txns_geo = txns.merge(props_lookup, on="property_id", how="left")
risk_pivot = txns_geo.pivot_table(index="buyer_type", columns="city", values="aml_flag", aggfunc="mean") * 100
fig_risk_heat = px.imshow(
    risk_pivot, text_auto=".0f", aspect="auto",
    color_continuous_scale=[[0, "#F0FDF4"], [0.5, ACCENT], [1, DANGER]],
    labels=dict(color="Flag rate %"),
    title="AML flag rate (%) — buyer entity type vs. city",
)
fig_risk_heat.update_layout(height=520, coloraxis_colorbar=dict(title=""))
...
with tab1:
    ...
    with c3:
        st.metric("Cities covered", f"{txns_geo['city'].nunique()}")
    with c4:
    ...
    top_pattern = type_counts.iloc[0]
    flagged_city = txns[txns["aml_flag"] == 1].merge(
        pd.read_csv(os.path.join(BASE, "data", "properties.csv"))[["property_id", "city"]], on="property_id", how="left"
    )["city"].value_counts()
    cash_flagged_avg = txns[txns["pattern_types"] == "high_cash"]["cash_component_pct"].mean()
    insight_grid([
        ("Most common typology", f"<b>{top_pattern['pattern']}</b> accounts for <b>{top_pattern['count']}</b> of all flagged transactions.", "danger"),
        ("City with most flags", f"<b>{flagged_city.index[0]}</b> has the highest count of flagged transactions ({flagged_city.iloc[0]}).", "warning"),
        ("Avg cash component when flagged", f"High-cash-flagged deals average <b>{cash_flagged_avg:.0f}%</b> cash — well above what Sections 269SS/269ST practically allow.", "primary"),
    ])

    section_title("🏙️", "Flagged transactions by city")
    city_flag_full = flagged_city.reset_index()
    city_flag_full.columns = ["city", "flagged_count"]
    total_by_city = txns_geo["city"].value_counts().reset_index()
    total_by_city.columns = ["city", "total_count"]
    city_flag_full = city_flag_full.merge(total_by_city, on="city", how="left")
    city_flag_full["flag_rate"] = city_flag_full["flagged_count"] / city_flag_full["total_count"] * 100
    city_flag_full = city_flag_full.sort_values("flagged_count")
    fig_city_flags = px.bar(
        city_flag_full, x="flagged_count", y="city", orientation="h",
        color="flag_rate", color_continuous_scale=[[0, "#FEF3C7"], [1, DANGER]],
        title="Flagged transaction count by city (color = flag rate %)", text="flagged_count",
    )
    fig_city_flags.update_traces(textposition="outside")
    fig_city_flags.update_layout(height=650, xaxis_title="Flagged transactions", yaxis_title="",
                                    plot_bgcolor="rgba(0,0,0,0)", coloraxis_colorbar=dict(title="Flag rate %"))
    st.plotly_chart(fig_city_flags, use_container_width=True)
...
with tab2:
    ...
    st.plotly_chart(
        network_graph_chart(G, ring_entities, title="Transaction network (red = ring member)", height=700),
        use_container_width=True,
    )

    if sccs:
        ring_sizes = [len(s) for s in sccs]
        insight_grid([
            ("Ring detection rate", f"<b>{len(sccs)}</b> rings found among <b>{G.number_of_nodes():,}</b> entities — a <b>{len(sccs)/G.number_of_nodes()*100:.2f}%</b> hit rate that would be invisible to per-transaction rules alone.", "danger"),
            ("Largest ring", f"The biggest detected ring has <b>{max(ring_sizes)}</b> entities trading among themselves.", "warning"),
        ])
        section_title("🔍", "Detected rings")
