...
section_title("🌡️", "Price heatmap: city tier × property type")
heat_source = rentable if is_rent else props
heat_val_col = "rent_per_sqft" if is_rent else "price_per_sqft"
if is_rent:
    heat_source = heat_source.copy()
heat_pivot = heat_source.pivot_table(index="property_type", columns="city_tier", values=heat_val_col, aggfunc="mean")
fig_heat = px.imshow(
    heat_pivot, text_auto=".0f", aspect="auto",
    color_continuous_scale=[[0, "#EFF6FF"], [0.5, PRIMARY_LIGHT], [1, PRIMARY]],
    labels=dict(color=f"Avg {target_label}"),
    title=f"Average {target_label} — property type vs. city tier",
)
fig_heat.update_layout(height=480, coloraxis_colorbar=dict(title=""))
...
with tab1:
    ...
    section_title("🏙️", f"Full city ranking — average {target_label}")
    city_source = rentable if is_rent else props
    city_val_col = "rent_per_sqft" if is_rent else "price_per_sqft"
    city_agg_full = city_source.groupby(["city", "city_tier"])[city_val_col].mean().reset_index().sort_values(city_val_col)
    fig_city_full = px.bar(
        city_agg_full, x=city_val_col, y="city", color="city_tier", orientation="h",
        title=f"Average {target_label} across all 18 cities",
        color_discrete_sequence=[PRIMARY, PRIMARY_LIGHT, ACCENT], text=city_val_col,
    )
    fig_city_full.update_traces(texttemplate="₹%{text:,.0f}", textposition="outside")
    fig_city_full.update_layout(height=700, xaxis_title=f"Avg {target_label} (₹)", yaxis_title="", plot_bgcolor="rgba(0,0,0,0)")
    st.plotly_chart(fig_city_full, use_container_width=True)
    ...
    fig2.update_layout(plot_bgcolor="rgba(0,0,0,0)", height=520)
