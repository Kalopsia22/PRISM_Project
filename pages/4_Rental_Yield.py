...
from utils.styling import inject_css, page_header, section_title, insight_grid, model_metrics_bar, PRIMARY, ACCENT, SUCCESS
...
clf_path = os.path.join(BASE, "data", "yield_classification_metrics.csv")
if os.path.exists(clf_path):
    clf = pd.read_csv(clf_path).iloc[0]
    st.markdown("##### Classification framing: collaborative-filtering held-out evaluation")
    st.caption(
        "The risk-profile score above is a deterministic weighted formula, not a trained model — there's no "
        "non-circular way to compute accuracy/precision/recall/F1/ROC-AUC against a rule that would just be "
        "checked against its own inputs. The **collaborative-filtering layer is different**: it's a genuine "
        f"matrix-factorization model, evaluated the standard recsys way — {int(clf['n']):,} investor-locality "
        "interactions were held out, hidden from the SVD fit, and the reconstruction is scored on whether it "
        "correctly classifies real vs. simulated interest for interactions it never saw."
    )
    model_metrics_bar(clf["accuracy"], clf["precision"], clf["recall"], clf["f1"], clf["roc_auc"])
